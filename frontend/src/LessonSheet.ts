...
export function lessonHtml(item:Artifact,imageData:string){
 ...
 </style></head><body><p>AncientLens · Object-based lesson</p><h1>${e(item.title)}</h1><p>${e(item.date)} · ${e(item.collection)} collection</p><p class="print-help">${item.image?"This file includes its image and works offline.":"This text-only lesson works offline; no image is included."} Use your browser’s Print command to print or save as PDF.</p>${imageData?`<figure><img src="${imageData}" alt="${e(item.title)}"><figcaption>${e(item.institution)} · ${e(item.accession)} · ${e(item.image_license)} image. ${e(item.image_changes)}</figcaption></figure>`:""}<p class="caveat">Published source text · AncientLens scholarly review: unreviewed. Original spelling, typos and gaps are preserved. The image may not show every translated surface; passage alignment is unverified.${item.note?' '+e(item.note):''}</p><h2>Published source passages</h2>${item.passages.map((p,i)=>`<section><h3>Inscription ${i+1}</h3><blockquote>${e(p.text)}</blockquote>${p.source_text?`<p class="note">Source transcription: ${e(p.source_text)}</p>`:''}${p.remark?`<p class="note">Source note: ${e(p.remark)}</p>`:''}</section>`).join('')}<section><h2>Discussion prompts</h2><p class="note">AncientLens teaching aid, not source text.</p><ol><li>What does the published text say about this object?</li><li>${item.image?"Which details can you identify in the image?":"Which words or gaps indicate uncertainty in the published excerpt?"}</li><li>What remains uncertain or outside this excerpt?</li><li>Cite the published source and separate your interpretation from its wording.</li></ol></section><section><h2>Source and reuse</h2><p class="citation">${e(citation(item))}</p><p class="note">Private research notes are not included.</p></section></body></html>`;
}
